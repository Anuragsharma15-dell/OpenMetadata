...
- **Ingestion Framework**: This is a pluggable framework for ingesting metadata from various sources and tools to the metadata store. It supports 120+ connectors for data warehouses, databases, dashboard services, messaging services, pipeline services, and more.
...
**AI & MCP Integration**: Connect LLMs and AI agents directly to your metadata using OpenMetadata's built-in [MCP (Model Context Protocol) Server](https://open-metadata.org/mcp), introduced in v1.8.0. Compatible with Claude, Cursor, OpenAI, and any MCP client. Agents can search assets, read lineage, manage glossaries, and automate data operations — all governed by OpenMetadata's authorization engine. [Get started with Claude Desktop →](https://docs.open-metadata.org/latest/how-to-guides/mcp/claude)
 
**Connectors**: Ingest metadata from various sources and tools using connectors. OpenMetadata supports 120+ connectors for data warehouses, databases, dashboard services, messaging services, pipeline services, and more.
...
|--------|-------------|------|
| 🐳 **Self-Host (Docker)** | Deploy OpenMetadata in your own environment | [Local Docker Deployment Guide](https://docs.open-metadata.org/quick-start/local-docker-deployment) |
| 🧪 **Live Sandbox** | Explore OpenMetadata instantly with sample data — no setup required | [Try the Sandbox](https://sandbox.open-metadata.org) |
| ☁️ **Free Managed Cloud** | Use OpenMetadata as a fully managed service with your own data | [Sign Up Free on Collate](https://cloud.getcollate.io/signup) |
 
## Prerequisites
 
Before setting up OpenMetadata locally, make sure you have the following installed:
 
| Component | Requirement | Used For |
|-----------|-------------|----------|
| Docker & Docker Compose | Latest stable | Local deployment |
| Python | 3.8 or higher | Ingestion Framework |
| Java | 17 | Backend server |
| Node.js | 16 or higher | Frontend UI |
 
> **Just exploring?** The [Live Sandbox](https://sandbox.open-metadata.org) and [Free Managed Cloud](https://cloud.getcollate.io/signup) options require **zero local setup**.
